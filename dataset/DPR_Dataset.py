...
class DenseRetrievalTrainDataset(Dataset):
    ...
    def __init__(self, data_path, max_context_length, max_question_length, tokenizer, hard_negative):
        self.max_context_length = max_context_length
        self.max_question_length = max_question_length
        self.tokenizer = tokenizer
        self.hard_negative = hard_negative

        if self.hard_negative:  # hard negative 설정
        ...
        if self.hard_negative:  # hard negative 설정
            ...
            if os.path.exists("hard_negative.csv"):
                ...
                for context in tqdm(data_context, total=len(data_context)):
                    passages = self.add_hard_negative_passage(context, self.corpus["context"], k=4)
                    for i in range(3):
                        try:
    ...
    def add_hard_negative_passage(self, query, corpus, k=3):
