...
class QuestionAnsweringSeq2SeqTrainer(Seq2SeqTrainer):
    def __init__(self, *args, eval_examples=None, post_process_function=None, gen_config, **kwargs):
    ...
    def predict(
        ...
        if self.post_process_function is not None and self.compute_metrics is not None:
            # Only the main node write the results by default
            eval_preds = self.post_process_function(predict_examples, predict_dataset, output)
            metrics = self.compute_metrics(eval_preds)
